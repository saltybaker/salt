# -*- coding: utf-8 -*-
'''
A salt engine which retransmits the messages from 
salt event bus to another ZMQ channel.
'''
...
from salt.performance.payloads import message_observed, job_results_end, job_return  # nopep8 pylint: disable=E501
...
def start(host='127.0.0.1',
          port=9900):
    '''
    Listen to events and resend the message queue throughput 
    statistics to the Aggregator.
    '''
...
class MessageProcessor(object):
    ...
    def _recend_performance_data(self, event):
        ...
        # [KN] Let's forcibly add the current master_id
        # (given that in raas environment there can be a number of them).
    ...
    def _extract_data(self, event):
        # [KN] Don't know exactly why the payload can have nested 'data' 
        # envelopes.
        # Since we know the structure of performance-related messages,
        # the following algorithm is quite safe.
        e = event
        while 'data' in e:
            e = e['data']
        e.pop('_stamp', None)
    ...
    def _process_job_return(self, event):
        ...
        minion_id = data['id']
        self.sender.send(job_return(jid,
                                    self.master_id,
                                    minion_id,
                                    TimestampProvider.get_now()))
